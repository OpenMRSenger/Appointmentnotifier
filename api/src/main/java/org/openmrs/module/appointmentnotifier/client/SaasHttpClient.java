 ...
 *
 * <p>The raw FHIR R4 JSON is POSTed with the {@code X-Messaging-Provider} header set to the value
 * of the {@code appointmentnotifier.messagingProvider} global property (e.g. {@code SwiftSend}).
 *
...
public class SaasHttpClient {
	...
	public boolean sendPayload(String endpoint, String provider,
		...
		return doPost(endpoint, fhirPayload, provider, encounterUuid);
	}

	// ── HTTP mechanics ────────────────────────────────────────────────────────

	private boolean doPost(String endpoint, String body, String provider, String encounterUuid) {
